...
$db = dbConnect();
print getSetting($db,"MUSIC_PATH") . "\n";
changeSetting($db,"MUSIC_PATH","/home/matt/Music");
print getSetting($db,"MUSIC_PATH") . "\n";
dbDisconnect($db);

// Reads database schema, puts it into sqlite database
function initDB($db){
	$sql_init = file_get_contents("db/mama.sql");
	$db->exec($sql_init);
}
...
function getSetting($dbconn, $key){
	...
	$stmt = $dbconn->prepare('SELECT value FROM Settings WHERE key = ?')
...
function changeSetting($db, $key, $newvalue){
...
function PgdbDisconnect($dbconn){
	pg_close($dbconn);
}
